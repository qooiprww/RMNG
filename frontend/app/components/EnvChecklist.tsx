...
export function EnvChecklist({
  ...
  return (
    <div className="space-y-3">
      ...
      {rows === null && !error ? (
        <p className="py-3 text-center text-xs text-slate-400">Running checks…</p>
      ) : null}
              ...
              <li key={row.id} className="flex items-start gap-2.5 px-3 py-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {!row.required ? (
                      <span className="shrink-0 rounded bg-slate-100 px-1.5 py-0.5 text-[10px] font-semibold text-slate-400">
                        optional
                      </span>
                    ) : null}
                  </div>
                  {row.detail ? (
                    <p
                      className={`mt-0.5 text-xs ${
                        row.ok ? "text-slate-400" : failMuted ? "text-amber-700" : "text-red-600"
                      }`}
                    >
                      {row.detail}
                    </p>
